...
exports.BlockSubsidy = exports.TestNetCoinbaseSubsidyOptions = exports.MainNetCoinbaseSubsidyOptions = void 0;
const bignumber_js_1 = __importDefault(require("bignumber.js"));
exports.MainNetCoinbaseSubsidyOptions = {
    eunosHeight: 894000,
    /**
     ...
     */
    eunosFoundationBurn: '26859289307829046',
    emissionReduction: 1658,
    emissionReductionInterval: 32690
};
exports.TestNetCoinbaseSubsidyOptions = {
    eunosHeight: 354950,
    /**
 ...
 */
class BlockSubsidy {
    constructor(options = exports.MainNetCoinbaseSubsidyOptions) {
        this.options = options;
        this.reductionBlockSubsidies = this.computeBlockReductionSubsidies();
        this.reductionSupplyMilestones = this.computeReductionSupplyMilestones(this.reductionBlockSubsidies);
    }
    /**
    ...
    getBlockSubsidy(height) {
        validateHeight(height);
        if (height === 0) {
            return new bignumber_js_1.default(this.options.genesisBlockSubsidy);
        }
        if (height < this.options.eunosHeight) {
            return new bignumber_js_1.default(this.options.preEunosBlockSubsidy);
        }
     ...
     */
    getPreEunosSupply(height) {
        let supply = new bignumber_js_1.default(this.options.genesisBlockSubsidy);
        supply = supply.plus(new bignumber_js_1.default(this.options.preEunosBlockSubsidy).times(height));
        return supply;
    }
    /**
     * Calculate post-eunos supply
     */
    getPostEunosSupply(height) {
        ...
        return this.reductionSupplyMilestones[reductionCount].plus(this.reductionBlockSubsidies[reductionCount].times(reductionRemainder));
    }
    computeReductionSupplyMilestones(reductionBlockSubsidies) {
        const preEunosTotalSupply = new bignumber_js_1.default(this.options.preEunosBlockSubsidy).times(this.options.eunosHeight - 1);
        const baseSupply = new bignumber_js_1.default(this.options.genesisBlockSubsidy)
        ...
        for (let i = 1; i < reductionBlockSubsidies.length; i++) {
            ...
            supplyMilestones[i] = previousMilestone.plus(previousBlockSubsidy.times(this.options.emissionReductionInterval));
        }
        return supplyMilestones;
    }
    computeBlockReductionSubsidies() {
        const subsidyReductions = [
            new bignumber_js_1.default(this.options.eunosBaseBlockSubsidy)
        ];
        while (true) {
            const previousSubsidy = subsidyReductions[subsidyReductions.length - 1];
            const amount = previousSubsidy.times(this.options.emissionReduction).div(100000).integerValue(bignumber_js_1.default.ROUND_DOWN);
            if (amount.isZero()) {
                subsidyReductions.push(new bignumber_js_1.default(0));
                break;
            }
...
exports.BlockSubsidy = BlockSubsidy;
function validateHeight(height) {
    if (height < 0 || height % 1 !== 0) {
        throw new Error('height must be a positive whole number');
    }
}
//# sourceMappingURL=BlockSubsidy.js.map