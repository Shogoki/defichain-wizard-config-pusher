"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Stats = void 0;
class Stats {
    constructor(client) {
        this.client = client;
    }
    /**
     * Get stats of DeFi Blockchain
     *
     * @return {Promise<StatsData>}
     */
    get() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.client.requestData('GET', 'stats');
        });
    }
    /**
     * Get reward distribution of DeFi Blockchain
     *
     * @return {Promise<RewardDistributionData>}
     */
    getRewardDistribution() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.client.requestData('GET', 'stats/rewards/distribution');
        });
    }
    /**
     * Get supply of DeFi Blockchain
     *
     * @return {Promise<SupplyData>}
     */
    getSupply() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.client.requestData('GET', 'stats/supply');
        });
    }
    /**
     * Get burn info of DeFi Blockchain
     *
     * @return {Promise<BurnData>}
     */
    getBurn() {
        return __awaiter(this, void 0, void 0, function* () {
            return yield this.client.requestData('GET', 'stats/burn');
        });
    }
}
exports.Stats = Stats;
//# sourceMappingURL=stats.js.map